                ...
                for (let i = 0; i < 2; i++) {
                    ...
                    ErrorLevels.forEach((keyword, severityLvl) => {
                        ...
                        if (silentOptions[i] !== true && severityLvl <= minimallvl)
                            assert.isTrue(stub.calledOnce, `Didn't add a log event on lvl "${keyword}" while it was required. Minimal log level was "${ErrorLevels[minimallvl]}" and silent was "${silentOptions[i]}"`);
                        else
                            assert.isFalse(stub.calledOnce, `Did add a log event on lvl "${keyword}" while this wasn't allowed. Minimal log level was "${ErrorLevels[minimallvl]}" and silent was "${silentOptions[i]}"`);
                        stub.reset();
                    });
                }
            }
        ...
        suite('Make sure that Call() handles a rejected promise of a reporter', () => {
            ...
            test('By generating new Error log item', done => {
                ...
                setTimeout(() => {
                    ...
                    assert.instanceOf(info.error, Error, 'Log error doesn\'t match expectations');
                    assert.deepEqual(info.error.message, `Reporter "Test Reporter", rejected report promise`, 'Log errormsg doesn\'t match expectations');
        ...
        suite('Make sure that Call() handels a reporter that timesout', () => {
            test('By Generating a new Warning Log Item', done => {
                ...
                setTimeout(() => {
                    ...
                    assert.instanceOf(info.error, Error, 'Log error doesn\'t match expectations');
                    assert.deepEqual(info.error.message, `REPORTER TIMEOUT: Reporter "${reporter2Instance.Name}" didn't response within "${reporter2Instance.TimeOut}" miliseconds. `, 'Log errormsg doesn\'t match expectations');
                    assert.deepEqual(info.data, {
